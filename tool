...
1、在startup_mntn_test_0002.py中找，因为用例调用了searchKey方法，searchKey方法在util.py中，在searchKey中先使用""adb shell dmesg""，在dmesg中找关键字，找不到就在/data/log/android_logskmsgcat-log中面找，最后返回结果;kmsgcat日志中要找到时间突变的位置，只找从结尾开始到时间突变为止的内容;
